        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                ...
                for (x = 0; (x < e.ClipRectangle.Right) && (position < waveData.Length); x++)
                {
                    ...
                    short low;
                    if (bytesPerSample>1)
                      low = BitConverter.ToInt16(waveData, (int)position);
                    else
                      low = waveData[position];
                    ...
                    for (n = position + bytesPerSample; (n < (position + bytesToProcess)) && (n < waveData.Length); n += bytesPerSample)
                    {
                        short sample;
                        if (bytesPerSample > 1)
                          sample = BitConverter.ToInt16(waveData, (int)n);
                        else
                          sample = waveData[n];
                    ...
                    {
                      lowPercent = (((float)low) / 256);
                      highPercent = (((float)high) / 256);
                    }
                    ...
                    myBuffer.Graphics.DrawLine(linePen, px, py, x, l);
                    px = x;
                    py = l;
 ...
 */ 
        ...
        public bool InsertAmpMouthMovements()
        {
          // check if any audio is present
          if (memoryStream.Length == 0) return false;
          ...
          if ((globalMin == 0) && (globalMax == 0)) return false;
